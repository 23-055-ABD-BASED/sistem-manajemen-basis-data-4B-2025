...
-- menampilkan keseluruhan isi dari masing-masing tabel
...
-- membuat view_umkm_detail
CREATE VIEW view_umkm_detail AS SELECT 
a.nama_usaha,
b.nama_lengkap,
c.nama_kategori,
d.nama_skala,
e.nama_kabupaten_kota,
a.tahun_berdiri
FROM umkm a
JOIN pemilik_umkm b ON a.id_pemilik = b.id_pemilik
JOIN kategori_umkm c ON a.id_kategori = c.id_kategori
...
-- membuat view_pemilik_dan_usaha
...
b.nama_usaha
FROM pemilik_umkm a
JOIN umkm b ON a.id_pemilik = b.id_pemilik;

SELECT * FROM view_pemilik_dan_usaha;

-- membuat view_produk_umkm
...
b.harga
FROM produk_umkm b
JOIN umkm a ON b.id_umkm = a.id_umkm;

SELECT * FROM view_produk_umkm;

-- membuat view_umkm_menengah
...
FROM umkm a
JOIN pemilik_umkm b ON a.id_pemilik = b.id_pemilik
JOIN skala_umkm c ON a.id_skala = c.id_skala
WHERE c.nama_skala = 'menengah';

SELECT * FROM view_umkm_menengah;


-- membuat view_umkm_per_kota
CREATE VIEW view_umkm_per_kota AS SELECT
a.nama_kabupaten_kota, COUNT(b.id_umkm) AS jumlah_umkm
FROM kabupaten_kota a
LEFT JOIN umkm b ON a.id_kabupaten_kota = b.id_kabupaten_kota
GROUP BY a.id_kabupaten_kota;
